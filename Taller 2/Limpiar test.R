...
# Dejar solo las descripciones necesarias
nombres_test <- colnames(test_personas)
tabla_nombres <- tabla_nombres[tabla_nombres$Nombre %in% nombres_test, ]
...
test_hogares <- test_hogares %>%rename(
  cuartos_totales= P5000,
  dormitorios= P5010)



test_hogares_subset <- test_hogares[, c("id", "cuartos_totales", "dormitorios", "Nper")]

# Unir train_h con train_hogares_subset por la columna 'id'
test_h <- merge(train_h, test_hogares_subset, by = "id", all.x = TRUE)
...
test_h <- test_h %>%
  mutate(max_educ = recode(Max_Nivel_Educativo,   
                           ...
                           `5` = "basica",    
                           `6` = "superior"))  
table(test_h$max_educ)

# Eliminar filas con NA en max_educ
test_h <- test_h[!is.na(test_h$max_educ), ]

# Crear las variables dummy para max_educ nuevamente
dummies <- model.matrix(~ max_educ - 1, data = test_h)
...
test_h <- cbind(test_h, dummies)


####Guardar la base
write.csv(test_h, file = "test_h.csv", row.names = FALSE)
